...
test('integrity and fraud controls reject unsafe device',async()=>{const {api,state}=makeDb();state.device.integrityStatus='FAILED';const engine=new RewardEngine(api,new FakeRedis());await assert.rejects(()=>engine.grant(req()),/integrity check failed/);state.device.integrityStatus='VERIFIED';state.device.riskScore=90;await assert.rejects(()=>engine.grant(req({idempotencyKey:'idem-223456'})),/fraud controls/);});
