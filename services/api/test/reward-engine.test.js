...
function makeDb(fraudOverrides={}){
  const state={
    ...
    reward:{id:'rw1',appId:'app1',sourceType:'ads',sourceId:'ad1',amount:50n,status:'CONFIRMED',createdAt:new Date()}, redemption:null, account:{id:'ca1',appId:'app1',userId:'u1',balance:0n,version:0n}, ledger:[], holds:[],
  };
  const api={
    ...
    fraudScores:{create:async({data})=>({id:'fs1',...data})}, fraudEvents:{create:async({data})=>({id:'fe1',...data})}, riskSignals:{createMany:async({data})=>({count:data.length})}, securityActions:{create:async({data})=>({id:'sa1',...data})}, rewardHolds:{create:async({data})=>{const hold={id:`rh-${state.holds.length+1}`,...data};state.holds.push(hold);return hold;}},
...
test('integrity and fraud controls reject unsafe device',async()=>{const {api,state}=makeDb();state.device.integrityStatus='FAILED';const engine=new RewardEngine(api,new FakeRedis());await assert.rejects(()=>engine.grant(req()),/integrity check failed/);state.device.integrityStatus='VERIFIED';const highRisk={deviceRiskScore:100,rewardsHour:20,rewardsDay:100,withdrawals:10,ipUsers:10,relationships:3,referrals:[{status:'REJECTED'},{status:'REJECTED'},{status:'REJECTED'}]};const highDb=makeDb(highRisk);const highEngine=new RewardEngine(highDb.api,new FakeRedis());await assert.rejects(()=>highEngine.grant(req({idempotencyKey:'idem-223456'})),/held for security investigation/);assert.equal(highDb.state.ledger.length,0);assert.equal(highDb.state.redemption,null);assert.equal(highDb.state.holds.length,1);assert.equal(highDb.state.holds[0].status,'HELD');});
