...
export interface AccessClaims { sub: string; appId: string; sid: string; exp: number; }

export async function authenticateUser(request: FastifyRequest, db: PrismaClient, secret: string): Promise<AccessClaims> {
  const header = request.headers.authorization;
  if (!header?.startsWith('Bearer ')) throw httpError(401, 'AUTH_REQUIRED', 'Authentication required');
  const claims = verifyAccessToken(header.slice(7).trim(), secret);
  const session = await db.userSessions.findUnique({ where: { id: claims.sid } });
  if (!session || session.userId !== claims.sub || session.appId !== claims.appId || session.status !== 'ACTIVE' || session.expiresAt.getTime() <= Date.now()) throw httpError(401, 'SESSION_INVALID', 'Session is invalid or expired');
...
export function verifyAccessToken(token: string, secret: string): AccessClaims {
  ...
  const encodedHeader = parts[0]; const encodedPayload = parts[1]; const signature = parts[2];
  if (!encodedHeader || !encodedPayload || !signature) throw httpError(401, 'INVALID_ACCESS_TOKEN', 'Invalid access token');
  ...
  try {
    ...
    const payload = JSON.parse(atob(normalized)) as Partial<AccessClaims>;
...
function httpError(statusCode: number, code: string, message: string): Error & { statusCode: number; code: string } { const error = new Error(message) as Error & { statusCode: number; code: string }; error.statusCode = statusCode; error.code = code; return error; }
