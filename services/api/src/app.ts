...
export function buildApp(config: AppConfig, dependencies: AppDependencies): FastifyInstance {
  ...
  registerSecurity(app, config); registerResponseHooks(app); registerErrorHandler(app); registerHealthRoutes(app, dependencies); registerAuthRoutes(app, config, dependencies.db, dependencies.redis); registerUserRoutes(app, config, dependencies.db); registerRewardRoutes(app, config, dependencies.db, dependencies.redis);
