...
export function registerProviderRewardRoutes(app: FastifyInstance, config: AppConfig, db: PrismaClient, redis: RedisConnection): void {
  ...
  app.register(async (api) => {
    api.get('/ads', async (request) => { const claims = await auth(request as Req); const rewards = await db.rewards.findMany({ where: { appId: claims.appId, sourceType: 'ads', status: 'CONFIRMED' }, orderBy: { createdAt: 'desc' } }); return ok(rewards.map((r: { id: string; sourceId: string | null; name: string; amount: bigint; metadata: unknown }) => ({ id: r.id, sourceId: r.sourceId, name: r.name, amount: r.amount.toString(), metadata: r.metadata }))); });
    api.get('/survey', async (request) => { const claims = await auth(request as Req); const items = await db.surveys.findMany({ where: { appId: claims.appId, status: 'ACTIVE' }, orderBy: { createdAt: 'desc' } }); return ok(items.map((s: { id: string; externalId: string; provider: string; title: string; rewardAmount: bigint }) => ({ id: s.id, externalId: s.externalId, provider: s.provider, title: s.title, rewardAmount: s.rewardAmount.toString() }))); });
    api.get('/offerwall', async (request) => { const claims = await auth(request as Req); const items = await db.offerwalls.findMany({ where: { appId: claims.appId, status: 'ACTIVE' }, orderBy: { createdAt: 'desc' } }); return ok(items.map((o: { id: string; externalId: string; provider: string; name: string }) => ({ id: o.id, externalId: o.externalId, provider: o.provider, name: o.name }))); });
