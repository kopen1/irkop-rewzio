...
interface WebhookBody { eventId?: string; event_id?: string; id?: string; eventType?: string; event_type?: string; providerReference?: string; provider_reference?: string; reference?: string; referenceId?: string; status?: string; [key: string]: unknown }
...
export function registerPayoutRoutes(app: FastifyInstance, config: AppConfig, db: PrismaClient, redis: RedisConnection): void {
  ...
  app.register(async (api) => {
    ...
    api.post<{ Params: { provider: string }; Body: WebhookBody }>('/webhooks/payout/:provider', async (request) => {
      ...
      return ok(await service.handleWebhook({ appId, providerCode: request.params.provider, signature, rawPayload, payload }), 'Webhook processed');
    });
  }, { prefix: '/api/v1' });
}
