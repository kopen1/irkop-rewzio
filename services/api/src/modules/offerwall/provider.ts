export interface OfferwallCallbackEvent {
  provider: string;
  eventId: string;
  userId?: string;
  appId: string;
  offerwallExternalId: string;
  eventType: string;
...
export interface OfferwallProvider {
  ...
  normalizeCallback(input: unknown): Promise<OfferwallCallbackEvent>;
}

export async function withProviderTimeout<T>(task: Promise<T>, timeoutMs = 5000): Promise<T> {
  let timer: ReturnType<typeof setTimeout> | undefined;
  try { return await Promise.race([task, new Promise<T>((_, reject) => { timer = setTimeout(() => reject(providerError('PROVIDER_TIMEOUT', 'Provider request timed out', 504)), timeoutMs); })]); }
