...
export class AuthService {
  ...
  async refresh(appId: string, refreshToken: string, ipAddress?: string): Promise<AuthResult> {
    ...
    const current = await this.db.refreshTokens.findUnique({ where: { tokenHash } });
    ...
    const next = await this.db.$transaction(async (tx) => {
      const created = await tx.refreshTokens.create({ data: { appId, userId: user.id, sessionId: current.sessionId, tokenHash: hashToken(rawNext), expiresAt: new Date(Date.now() + REFRESH_TTL_SECONDS * 1000) } });
      await tx.refreshTokens.update({ where: { id: current.id }, data: { revokedAt: new Date(), replacedById: created.id } });
      return created;
    });
    const sessionId = current.sessionId || next.id;
    this.audit.log('auth.refresh.rotated', { appId, userId: user.id, sessionId, ipAddress });
  ...
  async logout(refreshToken: string): Promise<void> {
    const token = await this.db.refreshTokens.findUnique({ where: { tokenHash: hashToken(refreshToken) } });
    if (!token) return;
    await this.db.$transaction(async (tx) => {
      await tx.refreshTokens.update({ where: { id: token.id }, data: { revokedAt: new Date() } });
      if (token.sessionId) await tx.userSessions.update({ where: { id: token.sessionId }, data: { status: 'REVOKED', revokedAt: new Date() } });
    });
  ...
  private async findOrCreatePhoneUser(appId: string, phone: string) {
    ...
    if (!user) user = await this.db.users.create({ data: { phone, phoneVerifiedAt: new Date(), status: 'ACTIVE', referralCode: randomBytes(8).toString('hex') } });
    await this.db.userApps.upsert({ where: { appId_userId: { appId, userId: user.id } }, create: { appId, userId: user.id, status: 'ACTIVE', firstSeenAt: new Date(), lastSeenAt: new Date() }, update: { lastSeenAt: new Date() } });
  ...
  private async issueSession(appId: string, userId: string, ipAddress?: string): Promise<AuthResult> {
    ...
    await this.db.userSessions.create({ data: { id: sessionId, appId, userId, tokenHash, status: 'ACTIVE', expiresAt, lastSeenAt: new Date(), ipAddress } });
    await this.db.refreshTokens.create({ data: { appId, userId, sessionId, tokenHash, expiresAt } });
  ...
  private createAccessToken(userId: string, appId: string, sessionId: string): string {
    ...
    return `${input}.${createHmac('sha256', this.config.jwtAccessSecret).update(input).digest('base64url')}`;
  }
...
function generateOtp(): string { return String(randomBytes(4).readUInt32BE(0) % 900000 + 100000); }
