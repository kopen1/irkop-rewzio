  ...
  private async applyDelta(input: LedgerInput): Promise<BalanceResult & { ledgerId: string }> {
    ...
    return this.db.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      const locked = await tx.$queryRaw<Array<{ id: string; balance: bigint; version: bigint }>>`
        SELECT "id", "balance", "version" FROM "coin_accounts"
        WHERE "id" = ${account.id} AND "appId" = ${input.appId} AND "userId" = ${input.userId}
        FOR UPDATE
      `;
      ...
      if (input.idempotencyKey) {
        const existing = await tx.coinLedger.findUnique({ where: { appId_idempotencyKey: { appId: input.appId, idempotencyKey: input.idempotencyKey } } });
        if (existing) {
          if (!sameLedgerRequest(existing, input)) throw coinError('IDEMPOTENCY_CONFLICT', 'Idempotency key was already used for a different transaction', 409);
      ...
      const ledger = await tx.coinLedger.create({
        data: {
          appId: input.appId, userId: input.userId, transactionType: input.transactionType, source: input.source,
          amount: input.amount, referenceId: input.referenceId, balanceBefore: current.balance, balanceAfter: nextBalance,
...
function positiveAmount(value: bigint): bigint {
  if (typeof value !== 'bigint' || value <= 0n || value > MAX_COIN_AMOUNT) throw coinError('INVALID_COIN_AMOUNT', 'Coin amount must be a positive integer', 400);
  return value;
}
function validateInput(input: LedgerInput): void {
  if (!input.userId || !input.appId || !input.transactionType || !input.source) throw coinError('INVALID_LEDGER_INPUT', 'userId, appId, transactionType and source are required', 400);
  if (typeof input.amount !== 'bigint' || input.amount === 0n || input.amount > MAX_COIN_AMOUNT || input.amount < -MAX_COIN_AMOUNT) throw coinError('INVALID_COIN_AMOUNT', 'Coin amount is outside the allowed range', 400);
  if (input.idempotencyKey !== undefined && (input.idempotencyKey.length < 8 || input.idempotencyKey.length > 255)) throw coinError('INVALID_IDEMPOTENCY_KEY', 'Invalid idempotency key', 400);
}
function sameLedgerRequest(existing: ExistingLedger, input: LedgerInput): boolean {
  return existing.userId === input.userId && existing.appId === input.appId && existing.transactionType === input.transactionType && existing.source === input.source && existing.amount === input.amount && (existing.referenceId ?? null) === (input.referenceId ?? null);
}
function coinError(code: string, message: string, statusCode = 500): Error & { code: string; statusCode: number } {
  const error = new Error(message) as Error & { code: string; statusCode: number }; error.code = code; error.statusCode = statusCode; return error;
}
