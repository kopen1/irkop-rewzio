...
export class ReferralService {
  ...
  async create(appId: string, referredUserId: string, code: string): Promise<{ referralId: string; status: string }> {
    ...
    const [referrerSession, referredSession, referrerDevice, referredDevice] = users;
  ...
  async qualify(appId: string, referralId: string, sessionId: string): Promise<{ referralId: string; status: string; rewarded: boolean }> {
    ...
    const [referrerDevices, referredDevices, referrerSessions, referredSessions, withdrawals] = await Promise.all([
    ...
    const result = await this.rewards.grant({ appId, userId: referral.referrerUserId, sessionId, sourceType: 'referral', sourceId: referralId, idempotencyKey: `referral:${referralId}`, metadata: { qualifyingRewardId: qualifying.id } });
...
function err(code: string, message: string, statusCode: number): Error & { code: string; statusCode: number } { const e = Object.assign(new Error(message), { code, statusCode }); return e; }
