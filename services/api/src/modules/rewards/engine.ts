...
export interface RewardRequest { userId: string; appId: string; sessionId: string; sourceType: RewardSource; sourceId?: string; idempotencyKey: string; metadata?: unknown; }
export interface RewardResult { rewardId: string; redemptionId: string; ledgerId: string; amount: bigint; balance: bigint; status: 'CONFIRMED'; replayed: boolean; }
interface RewardHooks { integrity?: (input: RewardRequest, device: { id: string; integrityStatus: string; riskScore: number }) => Promise<void>; fraud?: (input: RewardRequest, device: { id: string; integrityStatus: string; riskScore: number }) => Promise<void>; eligibility?: (input: RewardRequest, reward: { id: string; amount: bigint; status: string }) => Promise<void>; }
...
export class RewardEngine {
  ...
  async grant(input: RewardRequest): Promise<RewardResult> {
    ...
    if (existing) {
      if (existing.userId !== input.userId || existing.appId !== input.appId) throw rewardError('IDEMPOTENCY_CONFLICT', 'Idempotency key was already used for a different reward', 409);
      if (existing.status === 'CONFIRMED') {
        const ledger = await this.db.coinLedger.findFirst({ where: { appId: input.appId, referenceId: existing.id } });
    ...
    const rateCount = await this.redis.incrementWithExpiry(`reward:rate:${input.appId}:${input.userId}`, 60);
    if (rateCount > 30) throw rewardError('REWARD_RATE_LIMITED', 'Reward request rate limit exceeded', 429);

    const [user, session] = await Promise.all([this.db.users.findUnique({ where: { id: input.userId } }), this.db.userSessions.findUnique({ where: { id: input.sessionId } })]);
    ...
    return this.db.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      const redemption = await tx.rewardRedemptions.create({ data: { appId: input.appId, userId: input.userId, rewardId: reward.id, amount: reward.amount, status: 'PENDING', idempotencyKey: input.idempotencyKey } });
...
function validateRequest(input: RewardRequest): void { if (!input.userId || !input.appId || !input.sessionId || !input.sourceType) throw rewardError('INVALID_REWARD_REQUEST', 'userId, appId, sessionId and sourceType are required', 400); if (!REWARD_SOURCES.includes(input.sourceType)) throw rewardError('INVALID_REWARD_SOURCE', 'Unsupported reward source', 400); if (!input.idempotencyKey || input.idempotencyKey.length < 8 || input.idempotencyKey.length > 255) throw rewardError('INVALID_IDEMPOTENCY_KEY', 'Invalid idempotency key', 400); }
