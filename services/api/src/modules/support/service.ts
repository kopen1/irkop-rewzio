...
export class SupportService {
  ...
  async getTicket(appId: string, userId: string, ticketId: string, admin = false): Promise<unknown> {
    ...
    const [messages, attachments, actions] = await Promise.all([
      this.db.supportMessages.findMany({ where: { appId, ticketId }, orderBy: { createdAt: 'asc' } }),
      this.db.supportAttachments.findMany({ where: { appId, ticketId }, orderBy: { createdAt: 'asc' } }),
      admin ? this.db.supportActions.findMany({ where: { appId, ticketId }, orderBy: { createdAt: 'desc' } }) : Promise.resolve([]),
    ]);
  ...
  async createTicket(input: { appId: string; userId: string; categoryId: string; subject: string; body: string; priority?: 'LOW'|'NORMAL'|'HIGH'|'URGENT'; idempotencyKey: string }): Promise<unknown> {
    ...
    const existing = await this.db.supportTickets.findFirst({ where: { appId: input.appId, userId: input.userId, subject: input.subject.slice(0, 160) } });
  ...
  async addAttachment(input: { appId: string; userId: string; ticketId: string; messageId?: string; fileName: string; mimeType: string; sizeBytes: number; fileUrl: string }): Promise<unknown> {
    ...
    if (input.messageId) {
      const message = await this.db.supportMessages.findFirst({ where: { appId: input.appId, id: input.messageId, ticketId: input.ticketId } });
      if (!message) throw error(404, 'SUPPORT_MESSAGE_NOT_FOUND', 'Support message not found');
    }
...
function validateAttachment(fileName: string, mimeType: string, sizeBytes: number, fileUrl: string) {
  if (!fileName || fileName.length > 255 || fileName.includes('..') || /[\\/]/.test(fileName) || fileName.includes('\0')) throw error(400, 'INVALID_FILE_NAME', 'Invalid attachment file name');
  if (!ALLOWED_MIME.has(mimeType)) throw error(400, 'UNSUPPORTED_FILE_TYPE', 'Unsupported attachment type');
  if (!Number.isInteger(sizeBytes) || sizeBytes <= 0 || sizeBytes > MAX_ATTACHMENT) throw error(400, 'FILE_TOO_LARGE', 'Attachment exceeds the 10 MB limit');
  if (!/^https:\/\//i.test(fileUrl)) throw error(400, 'INVALID_FILE_URL', 'Attachment storage URL must use HTTPS');
}
