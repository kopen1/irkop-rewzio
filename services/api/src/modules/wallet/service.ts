...
type Capacity = {
  max_wd_users_per_day: number;
  max_wd_transactions_per_day: number;
  max_payout_amount_per_day: bigint;
  max_wd_per_user_per_day: number;
  max_wd_amount_per_user_per_day: bigint;
  reset_time: string;
...
export class WalletWithdrawalService {
  ...
  async getWithdrawal(appId: string, userId: string, id: string) {
    const withdrawal = await this.db.withdrawals.findFirst({ where: { appId, userId, id } });
  ...
  async requestWithdrawal(input: { appId: string; userId: string; sessionId: string; methodId: string; amount: bigint; idempotencyKey: string; ipAddress?: string; deviceId?: string }) {
    ...
    if (existing) {
      if (existing.appId !== input.appId || existing.userId !== input.userId || existing.withdrawalMethodId !== input.methodId || existing.amount.toFixed(0) !== input.amount.toString()) throw error(409, 'IDEMPOTENCY_CONFLICT', 'Idempotency key was already used for a different withdrawal');
      return existing;
    }
    ...
    return this.db.$transaction(async (tx) => {
      ...
      if (!account) throw error(400, 'WALLET_NOT_INITIALIZED', 'Wallet has no coin account');
      ...
      const lockedBalance = BigInt(wallet.lockedBalance.toFixed(0));
      const available = account.balance / COINS_PER_IDR - lockedBalance;
      ...
      if (capacity.enable_daily_limit) {
        ...
        const userRows = rows.filter((row) => row.userId === input.userId);
        const total = sumAmounts(rows);
        const userTotal = sumAmounts(userRows);
  ...
  private async createLockedWithdrawal(tx: Tx, input: { appId: string; userId: string; methodId: string; amount: bigint; idempotencyKey: string }, wallet: { lockedBalance: DecimalLike }, available: bigint, status: 'APPROVED' | 'PENDING_REVIEW' | 'PENDING_CAPACITY', riskScore: number) {
    ...
    const previousLocked = BigInt(wallet.lockedBalance.toFixed(0));
    ...
    try {
      return await tx.withdrawals.create({ data: { appId: input.appId, userId: input.userId, withdrawalMethodId: input.methodId, amount, fee: '0', netAmount: amount, currency: CURRENCY, status, idempotencyKey: input.idempotencyKey, lockedAt: now, approvedAt: status === 'APPROVED' ? now : undefined, riskScore } });
    } catch (cause) {
      await tx.wallets.update({ where: { appId_userId: { appId: input.appId, userId: input.userId } }, data: { availableBalance: available.toString(), lockedBalance: previousLocked.toString(), version: updated.version } });
      throw cause;
    }
  }
  ...
  private async syncWalletInTransaction(tx: Tx, appId: string, userId: string) {
    ...
    const available = coinBalance / COINS_PER_IDR - BigInt(wallet.lockedBalance.toFixed(0));
    if (available < 0n) throw error(409, 'WALLET_INCONSISTENT', 'Wallet balance is inconsistent');
  ...
  private async loadCapacity(appId: string): Promise<Capacity> {
    const setting = await this.db.appSettings.findUnique({ where: { appId_key: { appId, key: DAILY_CAPACITY_KEY } } });
...
function dailyWindow(resetTime: string): { start: Date; end: Date } {
