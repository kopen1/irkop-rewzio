...
for (const m of schema.matchAll(/model\s+(\w+)\s*\{([\s\S]*?)\n\}/g)) {
  ...
  for (const line of body.split('\n')) {
    ...
    if (s.startsWith('@@') || s.includes('@relation(')) continue;
...
for (const line of constraints.split('\n')) {
  ...
  const entry = {
    cols: cols.split(',').map(x => x.trim().replaceAll('"', '')),
    refTable,
    refCols: refCols.split(',').map(x => x.trim().replaceAll('"', '')),
  };
...
function defaultSql(attrs) {
  if (/default\(now\(\)\)/.test(attrs)) return " DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now'))";
  const b = attrs.match(/@default\((true|false)\)/);
  if (b) return ` DEFAULT ${b[1] === 'true' ? 1 : 0}`;
  const n = attrs.match(/@default\((-?\d+)\)/);
  if (n) return ` DEFAULT ${n[1]}`;
  return '';
}

const out = ['-- GENERATED from Prisma schema. Do not edit manually.', 'PRAGMA foreign_keys = ON;'];
for (const model of models) {
  ...
  const cols = model.fields.map(f => {
    ...
    const d = defaultSql(f.attrs); if (d) parts.push(d);
    return parts.join(' ');
  });
